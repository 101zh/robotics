...
//Variables
double error = calvin.heading(deg) - angle;
double totalError = 0;
double lastError = 0;
double motorPower = 0;
  ...
  while (error < -1 || error > 1) {
    ...
    error = calvin.heading(deg) - angle;
    totalError += error;
    ...
    rightFront.spin(reverse, motorPower, percent);
    rightBack.spin(reverse, motorPower, percent);
...
double motorPower = 0;
  ...
  while (error < -1 || error > 1) {
    ...
    error = calvin.heading(deg) - /*360 +*/ angle;
